...
contract burn_token is SafeMath{
    ...
    uint public start_time = 1625584548 ;//挖矿开始时间
    uint256 public UserMaxPower = 49990e3;
    ...
    uint public anti_bot = 500e8;//如果v1用户token余额小于这个值，不能挖矿
    address public requireToken = 0xDF0e293CC3c7bA051763FF6b026DA0853D446E38;//token合约地址
    address public daoAddress;
    address public startAddress;

    address public usdAddress = 0xDF0e293CC3c7bA051763FF6b026DA0853D446E38;//usd合约
    uint public basePrice = 3e5;
    ...
    mapping (address => address[]) public follower;//好友
    mapping (address => uint256) public buy_amount;//购买数量
    mapping (address => uint256) public sell_amount;//出售数量
    mapping (address => uint256) public rewardCount;//累计奖励
    mapping (address => bool) public isAutoBurn;//是否自动燃烧
    ...
    //可以把错误转入的token转出
    function withdrawOtherToken(address token,uint256 amount) public {
        require(msg.sender == owner);
        require(token != address(this));
        require(token != requireToken);
        require(token != usdAddress);
        TransferHelper.safeTransfer(token,msg.sender,amount);
    }
    ...
    function upgrade() public{
        ...
        uint256 bt_power = iburn_token(0x90EC663CA289ef14a50e2355de19C9de6575Cd7f).power(msg.sender);
        ...
        uint256 bt_last_miner = iburn_token(0x90EC663CA289ef14a50e2355de19C9de6575Cd7f).last_miner(msg.sender);
        if(bt_last_miner > 0)//更新上次挖矿时间
        {
            last_miner[msg.sender] = bt_last_miner;
        }

        address bt_invite  = iburn_token(0x90EC663CA289ef14a50e2355de19C9de6575Cd7f).invite(msg.sender);
        if(bt_invite != address(0))
        {
            invite[msg.sender] = bt_invite;//记录邀请人
            inviteCount[bt_invite] += 1;//邀请人的下级数加一
            follower[msg.sender].push(bt_invite);
        }
    }
    ...
    //新用户可以低价购买一个v2
    function buyPower() public{
        require(power[msg.sender] >= 90e3 && power[msg.sender] <= UserMaxPower);
        TransferHelper.safeTransferFrom(usdAddress, msg.sender, address(this), basePrice * 134);
        //如果余额够，直接出售134个
        if(balanceOf[address(this)] >= 134e3){
            TransferHelper.safeTransfer(address(this),msg.sender,134e3);
            burn(134e3);
        }
        else{
            //如果需要混合燃烧，从质押里面扣
            if (burnTokenAmount > 0) {
                require(TokenBalanceOf[msg.sender][requireToken] >= burnTokenAmount * 134e3 / 10000,"token insufficient");
                TokenBalanceOf[msg.sender][requireToken] -= burnTokenAmount * 134e3 / 10000;
                TransferHelper.safeTransfer(requireToken,address(0x0000000000000000000000000000000000000010), burnTokenAmount * 134e3 / 10000);
            }
            power[msg.sender] += 402e3;
            totalPower += 402e3;
        }
        buy_amount[msg.sender] += 134e3;
    }

    //出售给系统
    function sell() public{
        uint256 balance = balanceOf[msg.sender];
        uint256 userPower = power[msg.sender];
        uint256 fee= transfer_fee(msg.sender,balance);
        uint256 sellbtamount = balance - fee;
        require(buy_amount[msg.sender] >= sell_amount[msg.sender] + sellbtamount,"out buy");
        uint256 getUSDTamount = sellbtamount * basePrice / 1000;
        buy_amount[msg.sender] += sellbtamount;
        transfer(address(this),sellbtamount);
        TransferHelper.safeTransfer(usdAddress,msg.sender,getUSDTamount);
    }

    function SetBasePrice(uint _value) public returns (bool success)  {
        require(msg.sender == owner);
        basePrice = _value;
        return true;
    }
    ...
    function burn(uint256 _value) public returns (bool success)  {
		...
		require(_value > 0);
        require(power[msg.sender] + _value * 3 <= UserMaxPower,"UserMaxPower");
    ...
    function reward_upline(uint256 _value) private returns (bool success){
        ...
        if(invite[msg.sender] != address(0))
        {
            ...
            else if(power[invite1] < 2000 * 1e3)
            {
                scale = 5;
            }
            ...
            if(invite[invite1] != address(0))
            {
                ...
                else if(power[invite2] < 2000* 1e3)
                {
                    scale = 1;
                }
    ...
    function setStartAddress(address new_start) public {
        require(msg.sender == owner);
        startAddress = new_start;
    }
    ...
    // 设置用户最高算力
    function setUserMaxPower(uint _value) public{
        require(msg.sender == owner);
        UserMaxPower = _value;
    }
    ...
    function follower_amount(address account) public view returns(uint256 amount){
        return follower[account].length;
    }
    ...
    function registration(address invite_address) public returns (bool success){
        ...
        if(invite_address != startAddress) //如果邀请人不是起始地址，必须也有邀请人
        {
            require(invite[invite_address] != address(0),"inviter need have inviter");
        }
        follower[msg.sender].push(invite_address);
        invite[msg.sender] = invite_address;//记录邀请人
        inviteCount[invite_address] += 1;//邀请人的下级数加一
        return true;
    }

    function mintAndBurnAll() public returns (bool success){
        mint();
        burn(balanceOf[msg.sender]);
        return true;
    }

    function setAutoBurn() public returns (bool success){
        isAutoBurn[msg.sender] = !isAutoBurn[msg.sender];
        return true;
    }
    ...
    function mint() public returns (bool success){
        ...
        //如果开启自动燃烧，每次挖矿前先燃烧一次全部余额
        if(isAutoBurn[msg.sender])
        {
            burn(balanceOf[msg.sender]);
        }
        uint8 scale = 20;// 万分之n
        ...
        else if(power[msg.sender] < 2000* 1e3)
        {
            scale = 50;
        }
